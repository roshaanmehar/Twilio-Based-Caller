...
export class OutreachService {
  ...
  // Initiate call using Eleven Labs
  async initiateCall(phoneNumber, businessName) {
    try {
      console.log(chalk.blue(`📞 Initiating call to ${phoneNumber} for ${businessName}`))

      const response = await axios.post(
        "https://api.elevenlabs.io/v1/convai/twilio/outbound-call",
        {
          agent_id: this.elevenLabsConfig.agentId,
          agent_phone_number_id: this.elevenLabsConfig.phoneNumberId,
      ...
      console.log(chalk.green(`✅ Call initiated successfully for ${businessName}`))
      return {
        success: true,
  ...
  async waitForCallCompletion(conversationId, maxWaitTime = 400) {
    const startTime = Date.now()
    const pollInterval = 10000 // 10 seconds
    let callCompleted = false
    let conversationDetails = null

    console.log(chalk.cyan(`⏰ Monitoring call ${conversationId} (max wait: ${maxWaitTime}s)`))
    ...
    while (!callCompleted && Date.now() - startTime < maxWaitTime * 1000) {
      try {
        ...
        console.log(chalk.yellow(`⚠️ Error polling conversation status: ${pollError.message}. Retrying...`))
        await new Promise((resolve) => setTimeout(resolve, pollInterval))
      }
    }

    if (!callCompleted) {
      console.error(chalk.red(`❌ Call monitoring timeout: Call did not complete within ${maxWaitTime} seconds`))

      // Try to get final conversation data from Eleven Labs even after timeout
      try {
        console.log(chalk.cyan(`🔍 Attempting to fetch final conversation data from Eleven Labs...`))
        const response = await axios.get(`https://api.elevenlabs.io/v1/convai/conversations/${conversationId}`, {
          headers: {
            "xi-api-key": this.elevenLabsConfig.apiKey,
          },
        ...
        conversationDetails = response.data
        console.log(chalk.blue(`📊 Final conversation status from Eleven Labs: ${conversationDetails.status}`))
      } catch (fetchError) {
        console.log(chalk.yellow(`⚠️ Could not fetch final conversation data: ${fetchError.message}`))
      }

      throw new Error(
        `Call did not complete within ${maxWaitTime} seconds - this is likely a timeout, not a final failure`,
      )
    }

    // Extract custom field data - FIXED to properly handle boolean values
    const analysisData = conversationDetails.analysis || {}
    const dataCollectionResults = analysisData.data_collection_results || {}
    const partnershipData = dataCollectionResults.isTheRestaurantPartneredWithInfinityClub || {}
    // Use !== undefined to properly handle false values
    const isPartneredWithInfinityClub = partnershipData.value !== undefined ? partnershipData.value : null
    ...
    console.log(chalk.magenta(`📊 Call Results Summary:`))
    console.log(chalk.white(`   Status: ${conversationDetails.status}`))
    console.log(chalk.white(`   Call Successful: ${callSuccessful ? "success" : "failure"}`))
    console.log(chalk.white(`   Duration: ${conversationDetails.metadata?.call_duration_secs || "N/A"} seconds`))
    console.log(chalk.white(`   Infinity Club Partnership: ${isPartneredWithInfinityClub}`))
...
Infinity Club`,
  ...
  // Update MongoDB record after outreach
  async updateRecord(
    databaseName,
    collectionName,
    recordId,
    ...
    try {
      ...
      await collection.updateOne(
        { _id: objectId },
        {
          $set: {
            ...
            "outreach.alignment.status": callResults.isPartneredWithInfinityClub,
      ...
      // Add call results to outreach data
  ...
  async processRecord(databaseName, collectionName, record) {
    try {
      console.log(chalk.cyan(`\n🔄 Processing record: ${record.businessname} (${record._id})`))
      ...
      // Initiate call
      const callInitResult = await this.initiateCall(phoneNumber, record.businessname)

      // Wait for call completion
      const callResults = await this.waitForCallCompletion(callInitResult.conversationId)
